...
Now make a nice formatted table with values rounded and only the values you want to include using the code in the practice.qmd file as a template.

Make sure to replace the following in the code

-   dataframe name (replace "df" with "hrvData")

-   continuous variable name (replace "Measurement" with your HRV variable of choice)

-   categorical variable name (replace "Group" with "test.period")
...
Pick which framework you want to use to compare among treatments (estimation statistics or NHST). Then copy the code from the practice.qmd and complete that analysis with the hrvData dataset. Again, make sure to do the following replacements in the code:
...
-   subjects variable name (replace "ID" with "subject")
...
-   Save this file and give it the name "dataIntro_yourname.qmd"
