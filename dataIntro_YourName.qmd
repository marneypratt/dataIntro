...
hrvData is a tidy data spreadsheet. There are 20 columns and 33 observations in each column. Each column has a column name and a more descriptive label that we added using the `set_variable_labels` function from the `labelled` package. the metadata dataset has the list of all the variables including the units.
...
Make sure to fill in the blank with your HRV variable name for the y-axis label (`ylab()` function). This will be the label on your Y-axis, so it can look nicer and should have the units included as needed.
