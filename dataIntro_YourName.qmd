---
title: "dataIntro_YourName"
subtitle: "Practice Data Analysis Results"
...
Make sure you update the following items in the YAML (lines 1 to 10) above:

- title: replace YourName with your first and last name with no spaces (example: "dataIntro_SophiaSmith")
- author: replace "Your Name Here" with your full name with spaces (example: "Sophia Smith")
...
library(ggbeeswarm) ## for jittering points (you can remove this if you did not use a dotplot)
library(rstatix) #pipe friendly framework for basic stats
library(ggpubr) #publication ready plots & QQ plots
...
hrvData is a tidy data spreadsheet. There are 20 columns and 45 observations in each column. Each column has a column name and a more descriptive label that we added using the `set_variable_labels` function from the `labelled` package. the metadata dataset has the list of all the variables including the units.
...
#fill in the blank with the number of the subject ID you want to remove


df <- df |> 
  dplyr::filter(subject != __)
...
It is not always necessary to give the values of the descriptive statistics. Especially in a presentation (which is the way we will report results for the project in this course), you want to minimize text and it is usually better to show things visually (in a graph) rather than textually (in a table). 
...
Make sure to fill in the blank with your HRV variable name for the y-axis label (`ylab()` function). This will be the label on your Y-axis, so it can look nicer and should have the units included as needed. If you have milliseconds squared as the unit (any of the power variables), open the "axis.labels.R" script template and copy the appropriate line of code to replace the "ylab()" line below.
...
line.plot <- df  |>    
  ...
  ylab("___") +  #make sure to include units here as needed
...
effect.plot <- effectSize.results  |>    
  ...
  theme_classic(base_size = 14)  +  #sets the font size
   theme(
    ...
    axis.text.x = element_text(size = 7) #Set x axis label size
  ) +
  theme(legend.position = "none")   #controls legend/key
...
patch.plot <- line.plot + effect.plot +
...
ggsave(filename = "results/practiceDataResults.png",  
       plot = patch.plot,   #put the object name of your plot here
       height = 5, width = 8, units = "in", 
       dpi = 300)
```

The above code will save your effect size graph you made and call it "practiceDataResults.png". This file will appear in the results folder within dataIntro folder under the Files tab in the lower right quadrant of this window.

If you are working on the Smith server, you will first need to export this file to your computer:

-   Check the box next to the "practiceDataResults.png" file
